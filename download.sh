download_eval() {
    local name="$1"
    if [ -z "$name" ]; then
        echo "Usage: download_eval <images_subdir_under_samples>"
        return 1
    fi
...
download_eval download

rm -rf results/download_clip.json
rm -rf results/download_imagereward.json
rm -rf results/download_pick.json
rm -rf results/download_aesthetic.json
rm -rf results/download_hps.json
rm -rf results/download_mps.json