...
class AMED_predictor(torch.nn.Module):
    def __init__(
        ...
        self.fc_r = Linear(2 * noise_channels + bottleneck_output_dim, num_points)
        if self.scale_dir:
            self.fc_scale_dir = Linear(2 * noise_channels + bottleneck_output_dim, num_points)
        if self.scale_time:
            self.fc_scale_time = Linear(2 * noise_channels + bottleneck_output_dim, num_points)
        self.sigmoid = torch.nn.Sigmoid()
        
        # self.fc_r = Linear(2 * noise_channels, num_points)
        # if self.scale_dir:
        #     self.fc_scale_dir = Linear(2 * noise_channels, num_points)
        # if self.scale_time:
        #     self.fc_scale_time = Linear(2 * noise_channels, num_points)
        # self.sigmoid = torch.nn.Sigmoid()
...
class EPD_predictor(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__()
        assert sampler_stu in ['epd', 'amed', 'dpm', 'dpmpp', 'euler', 'ipndm']
        assert sampler_tea in ['heun', 'dpm', 'dpmpp', 'euler', 'ipndm']
        assert scale_dir >= 0
        assert scale_time >= 0
        self.dataset_name = dataset_name
    ...
    def forward(self, unet_bottleneck, step_idx, class_labels=None,):
        batch_size = unet_bottleneck.shape[0]
